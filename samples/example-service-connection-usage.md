...
This is the **post-POC** shape, after Customer publishes the upstream task as a
...
2. Fields:
   - **Connection name**: `github-app-customer-sandbox`
...
```yaml
steps:
  # Once the private extension is published under the Customer publisher,
  # this is the only line that changes vs. the POC.
  - task: create-github-app-token@1
    name: ghToken
    inputs:
      githubAppConnection: github-app-customer-sandbox
      owner: customer-sandbox
      repositories: automation-sandbox
      permissions: '{"contents":"read","metadata":"read"}'

  - bash: |
      gh api /repos/customer-sandbox/automation-sandbox | jq .default_branch
    env:
...
- The extension is packaged, signed, and published to a private Customer publisher, **and**
